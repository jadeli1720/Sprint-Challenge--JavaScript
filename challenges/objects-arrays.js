...
console.log(`The velociraptor diet type was ${velociraptor.diet}.`);
...
for(let i = 0; i < graduates.length; i++) {
  if (graduates[i].university === "Uni") {
    ...
    console.log (`None of the University names include the name Uni in them`)
  }
}
...
zooAnimals.forEach(items => {
  animalNames.push(`Names: ${items.animal_name}, Scientific: ${items.scientific_name}.`);
});
...
zooAnimals.map(items => {
  lowerCase.push(items.animal_name.toLowerCase());
});
...
*/
const lowerPopulation = zooAnimals.filter(count => count.population < 5);
...
*/
const populationTotal = zooAnimals.reduce((acc, items) => {
  acc = acc + items.population
  return acc
}, 0)
